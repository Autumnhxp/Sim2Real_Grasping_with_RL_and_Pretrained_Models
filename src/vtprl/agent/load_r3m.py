...
def load_r3m(modelid):
    ...
    if not os.path.exists(os.path.join(os.getcwd(), foldername)):
        print(f"Creating folder to download r3m modeL: {os.path.join(os.getcwd(), foldername)}")
        os.makedirs(os.path.join(os.getcwd(), foldername))
    modelpath = os.path.join(os.getcwd(), foldername, "model.pt")
    configpath = os.path.join(os.getcwd(), foldername, "config.yaml")
...
# Question at Autum: What is this? How is it different from load_r3m?
